...
export class AUTHENTICATION implements DurableObject {
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (request.method === "POST" && url.pathname === "/profile") {
			...
			const result = await this.env.AUTH_DB.prepare(
				...
				.bind(display_name || null, avatar_url || null, userId)
				.first<{ id: string }>();
			...
			return Response.json({ success: true, userId: result.id });
		}
		...
		return new Response("AUTHENTICATION DO", { status: 200 });
	}
}
